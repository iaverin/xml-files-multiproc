...
def create_zip_files(objects_in_xml, xml_files_in_zip: int, number_zip_files: int,  zip_files_dir: int, log_created_file: Callable = None) -> int:
    zip_files_created = 0
    for zip_file_index in range(1,number_zip_files+1):
        ...
        with zipfile.ZipFile(zip_file_name,"w") as z:
            for i in range (1, xml_files_in_zip+1):
                z.writestr(zipfile.ZipInfo(f"{i}.xml"), generate_xml_file_data(create_xml_tree(objects_in_xml)))
            zip_files_created += 1
        if log_created_file:
